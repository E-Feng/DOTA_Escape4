...
function barebones:DamageFilter(keys)
	...
	if inflictor then
		...
		--print("Damage filter ", damaging_ability:GetAbilityName())
		if damaging_ability and victim:IsHero() then
			--print(damaging_ability:GetName())
			local abilName = damaging_ability:GetName()
			if TableContains(SpellList, abilName) then
				--print("Changing to pure dmg")
				--print(keys.damagetype_const, keys.damage)
				keys.damagetype_const = DAMAGE_TYPE_PURE
				keys.damage = 1
...
function barebones:InventoryFilter(keys)
	--print("Inventory filter triggered")
	--PrintTable(keys)
