...
function barebones:CreepPatrol(unit, idx, turnDelay)
  -- For non patrolling units, stationary zombies
  if idx == 0 then
...
function barebones:MultiPatrolThinker(dummyUnit, entvals)
  ...
  --local m = -(goal.y - spawn.y)/(goal.x - goal.y)
  ...
  Timers:CreateTimer(1, function()
    if (GameRules.CLevel == level) or (GameRules.CLevel == 0) then
      local count = 0
